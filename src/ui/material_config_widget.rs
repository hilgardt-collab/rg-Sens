...
impl MaterialConfigWidget {
    ...
    fn create_theme_page(
        ...
        let theme_idx = match config.borrow().frame.theme {
        ...
        theme_dropdown.connect_selected_notify(move |dropdown| {
            ...
            config_clone.borrow_mut().frame.theme = match selected {
    ...
    fn default_graph_config_material() -> crate::ui::GraphDisplayConfig {
        ...
        config.line_color = Color { r: 0.24, g: 0.47, b: 0.96, a: 1.0 };  // Material Blue
        config.fill_mode = FillMode::Gradient;
        config.fill_gradient_start = Color { r: 0.24, g: 0.47, b: 0.96, a: 0.3 };
        config.fill_gradient_end = Color { r: 0.24, g: 0.47, b: 0.96, a: 0.0 };
        ...
        config.x_axis.grid_color = Color { r: 0.9, g: 0.9, b: 0.9, a: 1.0 };
        config.y_axis.show_grid = true;
        config.y_axis.grid_color = Color { r: 0.9, g: 0.9, b: 0.9, a: 1.0 };
    ...
    pub fn set_config(&self, config: &MaterialDisplayConfig) {
        *self.config.borrow_mut() = config.clone();
        ...
        if let Some(widgets) = self.theme_widgets.borrow().as_ref() {
            widgets.theme_dropdown.set_selected(match config.frame.theme {
                ThemeVariant::Light => 0,
                ThemeVariant::Dark => 1,
            });
