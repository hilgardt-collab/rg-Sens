...
pub fn save_config_with_app_config(
    ...
    // Use blocking_read() which waits for the lock. This blocks the GTK main thread
    // but only for the brief duration that the update manager holds a write lock
    // (typically < 1ms per panel). The previous try_read() + sleep(10ms) loop was worse:
    // it blocked for up to 500ms per panel, freezing all event processing.
