//! Art Nouveau configuration widget
//!
//! Provides a tabbed interface for configuring all aspects of the Art Nouveau display.
...
/// Art Nouveau color preset names
const ART_NOUVEAU_PRESETS: &[&str] = &[
    "Olive & Gold",
    "Teal & Copper",
    "Burgundy & Cream",
    "Peacock",
...
fn get_preset_theme(preset_idx: u32) -> Option<ComboThemeConfig> {
    match preset_idx {
        ...
        1 => Some(ComboThemeConfig {
            // Teal & Copper
            color1: Color::new(0.0, 0.502, 0.502, 1.0),   // Teal #008080
            color2: Color::new(0.722, 0.451, 0.200, 1.0), // Copper #B87333
            color3: Color::new(1.0, 1.0, 0.941, 1.0),     // Ivory #FFFFF0
            color4: Color::new(0.502, 0.0, 0.125, 1.0),   // Burgundy #800020
            gradient: LinearGradientSourceConfig {
                angle: 180.0,
                stops: vec![
                    ColorStopSource::theme(0.0, 1),
        ...
        2 => Some(ComboThemeConfig {
            // Burgundy & Cream
            color1: Color::new(0.502, 0.0, 0.125, 1.0),   // Burgundy #800020
            color2: Color::new(0.961, 0.961, 0.863, 1.0), // Cream #F5F5DC
            color3: Color::new(0.855, 0.647, 0.125, 1.0), // Gold #DAA520
            color4: Color::new(0.133, 0.369, 0.133, 1.0), // Forest green #228B22
            gradient: LinearGradientSourceConfig {
                angle: 180.0,
                stops: vec![
                    ColorStopSource::theme(0.0, 1),
        ...
        3 => Some(ComboThemeConfig {
            // Peacock
            color1: Color::new(0.0, 0.502, 0.502, 1.0),   // Teal #008080
            color2: Color::new(0.855, 0.647, 0.125, 1.0), // Gold #DAA520
            color3: Color::new(0.502, 0.0, 0.502, 1.0),   // Purple #800080
            color4: Color::new(0.0, 0.0, 0.545, 1.0),     // Dark blue #00008B
            gradient: LinearGradientSourceConfig {
                angle: 180.0,
                stops: vec![
                    ColorStopSource::theme(0.0, 1),
        ...
        4 => Some(ComboThemeConfig {
            // Autumn
            color1: Color::new(0.698, 0.133, 0.133, 1.0), // Rust #B22222
            color2: Color::new(0.420, 0.557, 0.137, 1.0), // Olive #6B8E23
            color3: Color::new(0.961, 0.961, 0.863, 1.0), // Cream #F5F5DC
            color4: Color::new(0.396, 0.263, 0.129, 1.0), // Brown #654321
            gradient: LinearGradientSourceConfig {
                angle: 180.0,
                stops: vec![
                    ColorStopSource::theme(0.0, 1),
...
struct BackgroundWidgets {
    ...
    wave_frequency_spin: SpinButton,
}
...
/// Art Nouveau configuration widget
pub struct ArtNouveauConfigWidget {
    container: GtkBox,
...
impl ArtNouveauConfigWidget {
    pub fn new(available_fields: Vec<FieldMetadata>) -> Self {
        ...
        let config = Rc::new(RefCell::new(ArtNouveauDisplayConfig::default()));
    ...
    pub fn get_config(&self) -> ArtNouveauDisplayConfig {
        self.config.borrow().clone()
    }

    pub fn get_config_rc(&self) -> Rc<RefCell<ArtNouveauDisplayConfig>> {
        self.config.clone()
    }

    pub fn set_config(&self, config: &ArtNouveauDisplayConfig) {
    ...
    pub fn set_theme(&self, theme: crate::ui::theme::ComboThemeConfig) {
        ...
        Self::refresh_theme_refs(&self.theme_ref_refreshers);
    ...
    fn create_theme_page(
        ...
        let preset_list = StringList::new(ART_NOUVEAU_PRESETS);
        let preset_dropdown = DropDown::new(Some(preset_list), Option::<gtk4::Expression>::None);
        preset_dropdown.set_selected(ART_NOUVEAU_PRESETS.len() as u32 - 1); // Default to "Custom"
    ...
    fn create_frame_page(
        ...
        let border_style_list = StringList::new(&["Vine", "Whiplash", "Floral", "Organic", "Peacock"]);
        ...
        let style_idx = match config.borrow().frame.border_style {
            BorderStyle::Vine => 0,
            BorderStyle::Whiplash => 1,
            BorderStyle::Floral => 2,
            BorderStyle::Organic => 3,
            BorderStyle::Peacock => 4,
        };
        ...
        border_style_dropdown.connect_selected_notify(move |dropdown| {
            ...
            config_clone.borrow_mut().frame.border_style = match selected {
                0 => BorderStyle::Vine,
                1 => BorderStyle::Whiplash,
                2 => BorderStyle::Floral,
                3 => BorderStyle::Organic,
                _ => BorderStyle::Peacock,
            };
            Self::queue_redraw(&preview_clone, &on_change_clone);
        });
        ...
        let corner_style_list = StringList::new(&["Flourish", "Leaf", "Spiral", "Bracket", "None"]);
        ...
        let corner_idx = match config.borrow().frame.corner_style {
            CornerStyle::Flourish => 0,
            CornerStyle::Leaf => 1,
            CornerStyle::Spiral => 2,
        ...
        corner_style_dropdown.connect_selected_notify(move |dropdown| {
            ...
            config_clone.borrow_mut().frame.corner_style = match selected {
                0 => CornerStyle::Flourish,
                1 => CornerStyle::Leaf,
                2 => CornerStyle::Spiral,
    ...
    fn create_background_page(
        ...
        let pattern_list = StringList::new(&["Solid", "Vines", "Leaves", "Waves", "Peacock"]);
        ...
        let pattern_idx = match config.borrow().frame.background_pattern {
            ...
            BackgroundPattern::Vines => 1,
            BackgroundPattern::Leaves => 2,
            BackgroundPattern::Waves => 3,
            BackgroundPattern::Peacock => 4,
        };
        ...
        pattern_dropdown.connect_selected_notify(move |dropdown| {
            ...
            config_clone.borrow_mut().frame.background_pattern = match selected {
                ...
                1 => BackgroundPattern::Vines,
                2 => BackgroundPattern::Leaves,
                3 => BackgroundPattern::Waves,
                _ => BackgroundPattern::Peacock,
            };
            Self::queue_redraw(&preview_clone, &on_change_clone);
        });
        ...
        // Wave frequency (for wave-based patterns)
        let freq_box = GtkBox::new(Orientation::Horizontal, 6);
        freq_box.append(&Label::new(Some("Wave Frequency:")));
        let wave_frequency_spin = SpinButton::with_range(1.0, 10.0, 0.5);
        wave_frequency_spin.set_value(config.borrow().frame.wave_frequency);
        wave_frequency_spin.set_hexpand(true);
        freq_box.append(&wave_frequency_spin);

        let config_clone = config.clone();
        let on_change_clone = on_change.clone();
        let preview_clone = preview.clone();
        wave_frequency_spin.connect_value_changed(move |spin| {
            config_clone.borrow_mut().frame.wave_frequency = spin.value();
            Self::queue_redraw(&preview_clone, &on_change_clone);
        });
        page.append(&freq_box);
    ...
    fn create_header_page(
        ...
        let style_list = StringList::new(&["Banner", "Arch", "Flourish", "None"]);
        ...
        let style_idx = match config.borrow().frame.header_style {
            HeaderStyle::Banner => 0,
            HeaderStyle::Arch => 1,
            HeaderStyle::Flourish => 2,
            HeaderStyle::None => 3,
        };
        ...
        header_style_dropdown.connect_selected_notify(move |dropdown| {
            ...
            config_clone.borrow_mut().frame.header_style = match selected {
                0 => HeaderStyle::Banner,
                1 => HeaderStyle::Arch,
                2 => HeaderStyle::Flourish,
                _ => HeaderStyle::None,
            };
            Self::queue_redraw(&preview_clone, &on_change_clone);
        });
    ...
    fn create_layout_page(
        ...
        let div_style_list = StringList::new(&["Vine", "Wave", "Tendril", "Line", "None"]);
        ...
        combo_config_base::rebuild_item_orientation_dropdowns(
            ...
            config,
            |c: &mut ArtNouveauDisplayConfig| &mut c.frame,
            on_change,
            preview,
        );
    ...
    fn refresh_all_widgets(&self) {
        ...
        if let Some(ref widgets) = *self.background_widgets.borrow() {
            ...
            widgets.wave_frequency_spin.set_value(config.frame.wave_frequency);
        }
    ...
    pub fn get_transferable_config(&self) -> crate::ui::combo_config_base::TransferableComboConfig {
        ...
        crate::ui::combo_config_base::TransferableComboConfig {
            ...
            item_spacing: 8.0, // Not configurable in Art Nouveau, use default
    ...
    pub fn apply_transferable_config(&self, transfer: &crate::ui::combo_config_base::TransferableComboConfig) {
        {
            ...
            // item_spacing not configurable in Art Nouveau
