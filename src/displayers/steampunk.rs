...
impl Displayer for SteampunkDisplayer {
    ...
    fn apply_config(&mut self, config: &HashMap<String, Value>) -> Result<()> {
        if let Some(config_value) = config.get("steampunk_config") {
            if let Ok(display_config) =
                serde_json::from_value::<SteampunkDisplayConfig>(config_value.clone())
            {
                self.inner.set_config(display_config.to_frame());
                return Ok(());
            }
            if let Ok(frame_config) =
                serde_json::from_value::<SteampunkFrameConfig>(config_value.clone())
            {
    ...
    fn get_typed_config(&self) -> Option<DisplayerConfig> {
        self.inner
            .get_config()
            .map(|frame| DisplayerConfig::Steampunk(SteampunkDisplayConfig::from_frame(frame)))
    }
}
