        ...
        for (i, &(item_x, item_y, item_w, item_h)) in layouts.iter().enumerate() {
            ...
            match item_config.display_as {
                ...
                ContentDisplayType::CoreBars => {
                    let animated_cores: Vec<f64> = core_bar_values
                        .get(&prefix)
                        .map(|anims| anims.iter().map(|a| a.current).collect())
                        .unwrap_or_default();

                    if let Err(e) = render_content_core_bars(
                        cr,
                        item_x,
                        item_y,
                        item_w,
                        ...
                        log::warn!("Failed to render core bars for {}: {}", prefix, e);
                    }
                }
                ...
                ContentDisplayType::Arc => {
                    ...
                    if let Err(e) = render_arc(
                        ...
                        log::warn!("Failed to render arc for {}: {}", prefix, e);
                    }
                    cr.restore()?;
                }
                ...
                ContentDisplayType::LevelBar => {
                    // LevelBar not yet implemented for Art Nouveau
                }
            }
        }
...
impl Displayer for ArtNouveauDisplayer {
        ...
        drawing_area.set_draw_func(move |_, cr, width, height| {
            ...
            if let Ok(data) = data_clone.lock() {
                ...
                let content_bounds = match render_art_nouveau_frame(cr, &data.config.frame, w, h) {
                    Ok(bounds) => bounds,
                    Err(e) => {
                        log::debug!("Art Nouveau frame render error: {}", e);
                        return;
                    }
                };
        ...
        ConfigSchema {
            options: vec![
                ConfigOption {
                    ...
                    description: "Art Nouveau border style".to_string(),
                    value_type: "string".to_string(),
                    default: serde_json::json!("vine"),
                },
                ConfigOption {
                    ...
                    value_type: "string".to_string(),
                    default: serde_json::json!("flourish"),
                },
                ConfigOption {
    ...
    fn apply_config(&mut self, config: &HashMap<String, Value>) -> Result<()> {
        ...
        if let Some(art_nouveau_config_value) = config.get("art_nouveau_config") {
            if let Ok(art_nouveau_config) = serde_json::from_value::<ArtNouveauDisplayConfig>(art_nouveau_config_value.clone()) {
                log::debug!(
                    "ArtNouveauDisplayer::apply_config - loaded {} groups, {} content_items",
                    art_nouveau_config.frame.group_count,
                    art_nouveau_config.frame.content_items.len()
                );
                if let Ok(mut display_data) = self.data.lock() {
                    display_data.config = art_nouveau_config;
                }
                return Ok(());
            } else {
                log::warn!("ArtNouveauDisplayer::apply_config - failed to deserialize art_nouveau_config");
            }
        }
    ...
    fn get_typed_config(&self) -> Option<crate::core::DisplayerConfig> {
        if let Ok(display_data) = self.data.try_lock() {
            log::debug!(
                "ArtNouveauDisplayer::get_typed_config - saving {} groups, {} content_items",
