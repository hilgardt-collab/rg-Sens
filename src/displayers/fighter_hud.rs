...
use crate::ui::fighter_hud_display::{
    render_fighter_hud_frame, calculate_group_layouts, draw_group_dividers,
    get_hud_color, FighterHudFrameConfig,
};
...
/// Full Fighter HUD display configuration
...
#[derive(Clone)]
struct DisplayData {
    config: FighterHudDisplayConfig,
    ...
    dirty: bool,
    // HUD-specific animation state
    scan_line_offset: f64,
}

impl Default for DisplayData {
    fn default() -> Self {
        Self {
            config: FighterHudDisplayConfig::default(),
            values: HashMap::new(),
...
impl FighterHudDisplayer {
    pub fn new() -> Self {
        Self {
            id: "fighter_hud".to_string(),
            name: "Fighter HUD".to_string(),
            data: Arc::new(Mutex::new(DisplayData::default())),
        }
    }

    /// Draw content items in a given area with HUD-style rendering
    ...
    fn draw_content_items(
        ...
        // Get HUD color for styling
        let hud_color = get_hud_color(&config.frame);
        ...
        for (i, &(item_x, item_y, item_w, item_h)) in layouts.iter().enumerate() {
            ...
            // Override colors with HUD color for military aesthetic
            slot_values.insert("bar_fill_color".to_string(), Value::Array(vec![
                Value::from(hud_color.r),
                Value::from(hud_color.g),
                Value::from(hud_color.b),
                Value::from(hud_color.a),
            ]));
...
impl Displayer for FighterHudDisplayer {
    ...
    fn create_widget(&self) -> Widget {
        ...
        drawing_area.set_draw_func(move |_, cr, width, height| {
            ...
            if let Ok(data) = data_clone.lock() {
                ...
                // Draw the Fighter HUD frame and get content bounds
                let content_bounds = match render_fighter_hud_frame(cr, &data.config.frame, w, h) {
                    Ok(bounds) => bounds,
                    Err(e) => {
                        log::debug!("Fighter HUD frame render error (usually harmless during layout): {}", e);
                        return;
                    }
                };
        ...
        glib::timeout_add_local(ANIMATION_FRAME_INTERVAL, {
            ...
            move || {
                ...
                let needs_redraw = if let Ok(mut data) = data_clone.try_lock() {
                    ...
                    if data.config.frame.scan_line_effect {
                        data.scan_line_offset += elapsed * 50.0;
                        if data.scan_line_offset > 100.0 {
                            data.scan_line_offset = 0.0;
                        }
                        redraw = true;
                    }
    ...
    fn draw(&self, cr: &Context, width: f64, height: f64) -> Result<()> {
        ...
        if let Ok(data) = self.data.try_lock() {
            ...
            render_fighter_hud_frame(cr, &data.config.frame, width, height)?;
            data.transform.restore(cr);
        }
        Ok(())
    }
    ...
    fn apply_config(&mut self, config: &HashMap<String, Value>) -> Result<()> {
        // Check for full fighter_hud_config first
        if let Some(config_value) = config.get("fighter_hud_config") {
            if let Ok(hud_config) = serde_json::from_value::<FighterHudDisplayConfig>(config_value.clone()) {
                if let Ok(mut display_data) = self.data.lock() {
                    display_data.config = hud_config;
                    // Clear stale animation data when config changes (lesson #14)
    ...
    fn get_typed_config(&self) -> Option<crate::core::DisplayerConfig> {
        ...
        if let Ok(display_data) = self.data.try_lock() {
            Some(crate::core::DisplayerConfig::FighterHud(display_data.config.clone()))
        } else {
            None
        }
    }
