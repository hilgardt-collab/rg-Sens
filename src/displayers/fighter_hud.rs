...
impl Displayer for FighterHudDisplayer {
    ...
    fn create_widget(&self) -> Widget {
        ...
        // Set up animation timer with HUD-specific scan line effect
        setup_combo_animation_timer_ext(
            &drawing_area,
            self.data.clone(),
            ...
            Some(|d: &mut DisplayData, elapsed: f64| -> bool {
                ...
                if d.config.frame.scan_line_effect {
                    d.scan_line_offset += elapsed * 50.0;
                    if d.scan_line_offset > 100.0 {
                        d.scan_line_offset = 0.0;
                    }
                    true
                } else {
                    false
